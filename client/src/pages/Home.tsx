    ...
    <div className={`site-shell exchange-shell theme-${theme} min-h-screen bg-[#071019] text-slate-100`}>
      ...
      <header className="sticky top-0 z-40 border-b border-white/[0.07] bg-[#071019]/90 backdrop-blur-xl">
        <div className="mx-auto flex h-[76px] max-w-[1480px] items-center gap-4 px-4 sm:px-6 lg:px-8">
          ...
          <nav className="hidden min-w-0 flex-1 items-center justify-center gap-0.5 overflow-hidden px-1 lg:flex">{NAV_ITEMS.filter((item) => item.label !== "Search").map((item) => <button key={item.label} onClick={() => { if (item.label === "Administrator") { window.location.href = "/administrator"; return; } if (item.label === "Profile") { window.location.href = "/profile"; return; }
              if (item.label === "All Online Trades") { window.location.href = "/all-trades"; return; } if (item.label === "Trending") { window.location.href = "/trending"; return; } if (item.label === "Search") { window.location.href = "/search"; return; } setActiveNav(item.label); document.getElementById(item.label === "Discover" ? "market-discovery" : item.label === "My portfolio" ? "portfolio" : item.label === "Orders" ? "open-orders" : "top-overview")?.scrollIntoView({ behavior: "smooth", block: "start" }); }} aria-label={item.label} title={item.label} className={`shrink-0 whitespace-nowrap rounded-lg px-2 py-2 text-[10px] font-semibold leading-4 transition ${activeNav === item.label ? "bg-white/[0.08] text-white" : "text-slate-400 hover:bg-white/[0.04] hover:text-slate-200"}`}>{item.label === "Search" ? <Search size={14} /> : item.label === "My portfolio" ? "Portfolio" : item.label === "All Online Trades" ? "Trades" : item.label === "Administrator" ? <span className="flex items-center gap-1.5">Administrator <LockKeyhole size={11} className="opacity-35" /></span> : item.label}</button>)}</nav>
