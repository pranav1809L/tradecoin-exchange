      ...
      <header className="sticky top-0 z-40 border-b border-white/[0.07] bg-[#071019]/90 backdrop-blur-xl">
        <div className="mx-auto flex h-[76px] max-w-[1480px] items-center gap-4 px-4 sm:px-6 lg:px-8">
          ...
          <nav className="hidden items-center gap-1 lg:flex">{NAV_ITEMS.map((item) => <button key={item.label} onClick={() => { if (item.label === "Profile") { window.location.href = "/profile"; return; }
              if (item.label === "All Online Trades") { window.location.href = "/all-trades"; return; } if (item.label === "Trending") { window.location.href = "/trending"; return; } if (item.label === "Search") { window.location.href = "/search"; return; } setActiveNav(item.label); document.getElementById(item.label === "Discover" ? "market-discovery" : item.label === "My portfolio" ? "portfolio" : item.label === "Orders" ? "open-orders" : "top-overview")?.scrollIntoView({ behavior: "smooth", block: "start" }); }} className={`rounded-lg px-3 py-2 text-xs font-semibold transition ${activeNav === item.label ? "bg-white/[0.08] text-white" : "text-slate-400 hover:bg-white/[0.04] hover:text-slate-200"}`}>{item.label}</button>)}</nav>
