...
type MarketDetail = {
  product: Product;
  buyOrders: Array<{ id: number; price: string; remainingQuantity: number; createdAt: string }>;
  sellOrders: Array<{ id: number; price: string; remainingQuantity: number; createdAt: string }>;
    ...
    <div className="min-h-screen bg-[#071019] text-slate-100">
      ...
      <header className="sticky top-0 z-40 border-b border-white/[0.07] bg-[#071019]/90 backdrop-blur-xl">
        <div className="mx-auto flex h-[76px] max-w-[1480px] items-center gap-4 px-4 sm:px-6 lg:px-8">
          ...
          <nav className="hidden items-center gap-1 lg:flex">{NAV_ITEMS.map((item) => <button key={item.label} onClick={() => setActiveNav(item.label)} className={`rounded-lg px-3 py-2 text-xs font-semibold transition ${activeNav === item.label ? "bg-white/[0.08] text-white" : "text-slate-400 hover:bg-white/[0.04] hover:text-slate-200"}`}>{item.label}</button>)}</nav>
          <div className="ml-auto flex items-center gap-3"><div className="hidden items-center gap-2 rounded-xl border border-white/10 bg-white/[0.035] px-3 py-2 text-xs text-slate-400 md:flex"><CircleDollarSign size={14} className="text-[#68e0c0]" /> <span className="font-mono">{money(wallet?.balance ?? "100000")}</span></div>{authLoading ? <div className="h-9 w-24 animate-pulse rounded-xl bg-white/[0.06]" /> : isAuthenticated ? <button onClick={() => logout()} className="flex items-center gap-2 rounded-xl border border-white/10 px-3 py-2 text-xs font-semibold text-slate-300 transition hover:border-white/20 hover:text-white"><span className="grid h-5 w-5 place-items-center rounded-full bg-[#d2a26f] text-[9px] font-black text-[#2c1b12]">{displayName.charAt(0).toUpperCase()}</span><span className="hidden sm:block">{displayName}</span><LogOut size={13} /></button> : <button onClick={() => startLogin()} className="rounded-xl bg-[#69ddbd] px-4 py-2.5 text-xs font-bold text-[#062a2b] transition hover:bg-[#8ceacd]">Sign in</button>}</div>
        </div>
      </header>
